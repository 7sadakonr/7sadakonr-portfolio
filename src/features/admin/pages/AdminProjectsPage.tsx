...
const AdminProjectsPage = () => {
  ...
  const load = async (isBackground = false) => {
    ...
    try { setProjects(await listAdminProjects()); setError(null) } catch (loadError) { setError(loadError instanceof Error ? loadError.message : 'Unable to load projects.') } finally { if (!isBackground) setIsInitialLoading(false) }
  }
  ...
  const deleteProject = async (project: ProjectRecord) => {
    ...
    try {
      ...
      // Remove from state optimistically so it doesn't wait for load(true)
  ...
  const move = async (project: ProjectRecord, direction: 'up' | 'down') => {
    setBusyId(project.id)
    try { await moveAdminProject(project.id, direction); await load(true) } catch (moveError) { setError(moveError instanceof Error ? moveError.message : 'Unable to reorder project.') } finally { setBusyId(null) }
  }
  ...
  return (
    <section className="admin-page">
      <div className="admin-page-heading">
        ...
        <button className="admin-button" type="button" onClick={() => navigate('/admin/projects/new', { state: { techSuggestions } })}>New project</button>
      </div>
      ...
      {isInitialLoading ? (
        ...
        <p className="admin-empty">No projects yet. Create the first one.</p>
      ) : (
        <div className="admin-project-list" aria-busy={busyId !== null}>
